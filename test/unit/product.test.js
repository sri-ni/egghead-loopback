...
describe('Product', function() {
  ...
  describe('Validation', function() {
    ...
    it('should reject a price below the minumum 99', function() {
      return Product.create({name: 'lowPrice', price: 98})
        .then(res => Promise.reject('Product should not be created'))
        .catch(err => {
          expect(err.message).to.contain(
            'Price should be higher than the minimal price in the DB'
          );
          expect(err.statusCode).to.be.equal(422);
        });
    });
  ...
  describe('Hooks', function() {
    it('should not allow adding product to non-existing category', function() {
